...
final class Centroids {
  /** Prevents construction of the centroid utility class. */
  private Centroids() {}
  ...
  static final class CentroidCodes {
    ...
    /** Stores exact centroids and their coarse and optional fine encodings. */
    ...
    /** Re-encodes every centroid after centroid values change. */
    void encodeAll() {
    ...
    /** Ranks candidate centroids using fine codes or exact distance. */
    ...
    static final class Routing {
      ...
      /** Allocates storage for a routing result. */
    ...
    static final class Scratch {
      ...
      /** Allocates reusable buffers for centroid routing. */
    ...
    /** Shortlists centroids by Hamming distance and verifies them exactly. */
    ...
    /** Tests whether the second centroid lies within the spill margin. */
  ...
  record CentroidGraph(
    ...
    static final int M = 16, EF_CONSTRUCTION = 64;
    static final int EF_MULTIPLIER = 2, MIN_EF = 32;
    ...
    /** Builds the centroid navigation graph. */
      ...
      Parallel.RangeTask insertRange =
          (from, to) -> {
            ...
            for (int idx = from + 1; idx <= to; idx++) {
              ...
              int[] kept = prune(codes, codes.centroids[node], visited, n);
    ...
    /** Adds fallback links until every centroid is reachable from the entry. */
    ...
    /** Appends a unique neighbor when capacity permits. */
    ...
    /** Selects a small diverse neighbor set from graph candidates. */
    ...
    /** Adds or replaces a reciprocal graph link. */
    ...
    /** Searches the centroid graph with coarse Hamming distance. */
    ...
    /** Writes the packed centroid graph. */
    ...
    /** Inserts a value while preserving min- or max-heap order. */
