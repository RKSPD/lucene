 ...
 *
 * <p>WHY THESE ARE NOT LEAKS. {@code Clustering.Parallel} holds ONE shared, eagerly created,
 * process-lifetime pool of daemon threads, because a split happens on nearly every build path and
 * the alternative is double-checked locking on a field each one touches. Being daemon threads they
 * never hold JVM exit open, and being a fixed pool their core workers are never reaped, so they
 * outlive whichever suite happened to trigger the first build — which is exactly what
 * randomizedtesting reports as a SUITE-scope leak.
 *
